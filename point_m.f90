  ...
  subroutine set_coef(this, param, nx, ny)
    ...
    if (ny <= param%my1) then
      if (nx == 1) then
        ...
        rr = sqrt(((nx - 1 ) * param%hx - param%xhs)**2 +&
      ...
      if (nx == 1) then
        ...
        rr = sqrt(((nx - 1 ) * param%hx - param%xhs)**2 +&
                  ((ny - 1 ) * param%hy - (param%yhs +   &
                  int(float(param%ny) * 2.0_DP/3.0_DP)))**2)
        this%T = param%t0hs * exp(-rr/param%r0hs)
        this%F = 1.0_DP
        this%Z = 0.1_DP * param%t0hs * exp(-rr/param%r0hs)
      end if
    ...
    if (ny == 1) then
      ...
      this%setYc => coefy_1
    else if ((ny < param%my1 .and. ny > 1) .or. &
      (ny > param%my2 .and. ny < param%ny)) then
